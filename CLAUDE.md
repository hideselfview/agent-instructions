...
better-structured. Same for value-laden size words ("minor", "trivial",
"substantial", "the big one") used to rank work — the user decides what
...
this?" The duplicate might already be handled upstream; the bug might be
a symptom of a deeper problem; the new abstraction might dissolve if the
right concept is introduced. The meta-question fires before the
implementation. *(See `principles/question-necessity.md`.)*
